...
def train(model: FederatedModel, private_dataset: FederatedDataset,
    ...
    if args.rand_dataset:
        ...
        while not is_ok:
            if model.args.dataset == 'fl_officecaltech':
                # domains_list = ['caltech', 'amazon', 'webcam', 'dslr'] # domains
                selected_domain_list = np.random.choice(domains_list, 
                ...
                # domains_list = ['mnist', 'usps', 'svhn', 'syn'] # domains
                ...
                # domains_list = ['photo', 'art', 'cartoon', 'sketch'] # domains
                selected_domain_list = np.random.choice(domains_list, size=args.parti_num, replace=True, p=None)
    ...
    # print(result)
    print(f"selected_domain_list for {args.parti_num} clients:")
    ...
    for epoch_index in range(Epoch):
        ...
        end_time = time.time()
        print('The ' + str(epoch_index) + ' Round Time:', round(end_time - start_time, 3))
        ...
        print('The ' + str(epoch_index) + ' Round Accuracy:', str(mean_acc), 
              'Method:', model.args.model, 'Epoch Loss:', str(epoch_loss))
        print(accs)
        print()
    ...
    print("loss of all rounds:", all_epoch_loss)
