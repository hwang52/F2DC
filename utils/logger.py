...
useless_args = ['pub_aug','public_len','public_dataset','structure', 
                'model', 'csv_log', 'device_id', 'seed',
...
class CsvWriter:
    ...
    def write_acc(self,accs_dict, mean_acc_list):
        acc_path = os.path.join(self.para_foloder_path, 'all_accuracy.csv')
        self._write_all_acc(acc_path, accs_dict)

        mean_acc_path = os.path.join(self.para_foloder_path, 'avg_accuracy.csv')
